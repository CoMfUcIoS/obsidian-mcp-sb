...
Completed personal task.`
  ...
  describe('End-to-End Workflows', () => {
    test('complete workflow: search, filter, and retrieve', async () => {
      ...
      const workNotes = vault.searchNotes('', { tags: ['work'] });
      expect(workNotes.length).toBe(2);

      // Step 2: Filter by type
      const meetings = vault.searchNotes('', { type: 'meeting' });
      expect(meetings.length).toBe(1);
      expect(meetings[0].title).toBe('meeting');

      // Step 3: Retrieve full note
      const fullNote = vault.getNote(meetings[0].path);
    ...
    test('category-based organization workflow', async () => {
      ...
      const workNotes = vault.searchNotes('', { category: 'work' });
      expect(workNotes.length).toBe(2);

      // Step 2: Get personal category notes
      const personalNotes = vault.searchNotes('', { category: 'personal' });
    ...
    test('path-based navigation workflow', async () => {
      ...
      const workNotes = vault.searchNotes('', { path: 'Work/**' });
      expect(workNotes.length).toBe(2);
      expect(workNotes.every(n => n.path.startsWith('Work'))).toBe(true);

      // Step 2: Get specific subdirectory
      const projectNotes = vault.searchNotes('', { path: 'Projects/**' });
      expect(projectNotes.length).toBe(1);
    ...
    test('fuzzy search workflow', async () => {
      ...
      const alphaResults = vault.searchNotes('alpha');
      expect(alphaResults.length).toBeGreaterThan(0);

      const meetingResults = vault.searchNotes('meeting');
    ...
    test('empty search returns all notes with limit', async () => {
      const results = vault.searchNotes('', { limit: 2 });
...
Puppet-related task.`
      ...
      const workTagged = vault.getNotesByTag('work');
      expect(workTagged.some(n => n.frontmatter.tags?.includes('work/puppet'))).toBe(true);

      // Exact match should work
      const puppetTagged = vault.getNotesByTag('work/puppet');
      expect(puppetTagged.length).toBeGreaterThan(0);
    });

    test('note retrieval by type methods', async () => {
      // Test getNotesByType
      const meetings = vault.getNotesByType('meeting');
      expect(meetings.length).toBe(1);
      expect(meetings[0].frontmatter.type).toBe('meeting');

      const projects = vault.getNotesByType('project');
      expect(projects.length).toBe(1);
      expect(projects[0].frontmatter.type).toBe('project');

      // Test getNotesByStatus
      const active = vault.getNotesByStatus('active');
      expect(active.length).toBeGreaterThan(0);
      expect(active.every(n => n.frontmatter.status === 'active')).toBe(true);

      const completed = vault.getNotesByStatus('completed');
